...
class PropertyService:
    ...
    async def create_property(
        ...
        try:
            ...
            if not location:
                raise NotFoundException("Location not found for the given location_id")
    ...
    async def get_existing_property(self, title: str, location_id: str):
        ...
        """
        try:
            location_ = await self.location_repository.get_location_by_id(location_id)
            if not location_:
                raise NotFoundException("Location not found for the given location_id")
            
            return await self.repository.get_existing_property(title, location_.id)
        except NotFoundException as e:
            raise e
        except Exception as e:
            logger.error(f"Error getting existing property: {str(e)}")
            raise InternalServerErrorException()
